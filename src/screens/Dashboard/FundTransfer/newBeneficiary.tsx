...
import {
    useFundTransfer,
    useGetBankList,
    useGetBeneficiaryName,
} from "./fundTransferServices";
...
const NewBeneficiary = (props: any) => {
    ...
    const getBeneficiaryName = async () => {
        if (
            ...
            setIsLoading(true);
            const data: any = await getBeneficiaryNameMutation.mutateAsync(
                payload
            );
            if (data.status === 200) {
                setIsLoading(false);
    ...
    const handleFundTransfer = async () => {
        if (validator.allValid()) {
            const payload: any = {
                ...
                transactionAmount: Number(txnData.transactionAmount)?.toFixed(
                    2
                ),
            };
            ...
            <div className={styles["input-container"]}>
                <div className={styles["fundTransferType"]}>
                    <CustomSelector
                        label={"Transfer Type"}
                        onChange={(e: any) =>
                            handleSelector(e, "", "", "fundTransferType")
                        }
                ...
                <div className={styles["sourceAccountNo"]}>
                    <CustomSelector
                        label={"Debit Account"}
                        onChange={(e) =>
                            handleSelector(e, "", "", "sourceAccountNo")
                        }
