    ...
    <div className={"main-container"}>
      ...
      <div className={styles['Account-content']}>
        <div className={styles["account-action-container"]}>
          {trackListData.length > 0 ? trackListData?.map(
            (
              {
                customerName,
                customerNumber,
              ...
              <div
                ...
                onClick={() => HandleViewDraft(dbsCustApplId,applicationType)}
              >
                <div className={styles["account-action-header"]}>{applicationType}</div>
                <div className={styles["account-action-content"]}>
                  <div className={styles["text-container"]}>
                    <p>Customer Name</p>
                    <b className={styles["green-text"]}>{customerName}</b>
                  </div>
                  
                  <div className={styles["text-container"]}>
                    <p>Customer Number</p>
                    <b className={styles["green-text"]}>{customerNumber}</b>
                  </div>
                  <div className={styles["text-container"]}>
                    <p>Created Date</p>
                    <b className={styles["green-text"]}>{systemTime}</b>
                  </div>
                  {
                    applicationType==="CREDIT"? <div className={styles["text-container"]}>
                    <p>CreditApplRefNo</p>
                    <b className={styles["green-text"]}>{credit_ref_no}</b>
                  </div>:""
                  }
                
                  <div className={styles["text-container"]}>
                    <p>Status</p>
                    <b className={styles["green-text"]}>{coreStatus==="S"?"Submitted":coreStatus==="I"?"Inactive":coreStatus==="A"?"Active":coreStatus==="C"?"Closed":""}</b>
                  </div>
                  <div></div>
                </div>
              </div>
            )
          ):<><p></p><p className={styles["nodata"]}>No Track to Show</p><p></p></>}
        </div>
