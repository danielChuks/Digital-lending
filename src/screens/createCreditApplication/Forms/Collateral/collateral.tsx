...
const Collateral: React.FC<any> = (props: any) => {
  ...
  const [collateraData, setCollateraData] = useState<CreditCollateralDocument>({
  ...
  useEffect(() => {
    if (props.collateralValue?.data) {
      setCollateraData({
        collateralDescription: props.collateralValue.data.collateralDescription,
        collateralMarketValue: props.collateralValue.data.collateralMarketValue,
        ...
        collateralCurrencyId: props.collateralValue.data.collateralCurrencyId,
      });
    }
  ...
  const handleCollateral = async (e: React.FormEvent) => {
    ...
    if (validator.allValid()) {
      ...
      setIsLoading(true);
      const data = await createCreditApplDraftMutation.mutateAsync(payload);
      if (data.status === 200) {
        setIsLoading(false);
        sessionStorage.setItem("dbsCustApplId", data.data.dbsCustApplId);
        populateCreditDraft();
        setCreditApplDataFields_context((prev) => ({
          ...prev,
          addDocumentFlag: false,
  ...
  const handleDeleteCollateral = async (e: React.FormEvent) => {
    ...
    setModelData({
      ...
      onOk: async () => {
        ...
        let arr: any;
        if (creditApplDataFields_context.collateralinfoData) {
          let temp = creditApplDataFields_context.collateralinfoData.splice(
            props.collateralValue?.index,
            1
          );

          if (temp.length > 0) {
            arr = [...creditApplDataFields_context.collateralinfoData];
          } else {
            arr = null;
          }
        }
          ...
          <div className={styles["input-container-split"]}>
            <div>
              <CustomSelector
                label={"Currency"}
                onChange={(e) =>
                  handleSelector(e, "", "", "collateralCurrencyId")
                }
