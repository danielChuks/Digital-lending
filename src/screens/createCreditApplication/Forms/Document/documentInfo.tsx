...
const DocumentInfo: React.FC<any> = (props: any) => {
  ...
  const fetPicklistData = async () => {
    ...
    if (data.data?.pickListMap?.FETCH_CREDIT_DOCUMENT_REF) {
      let temp: { value: any; label: any }[] = [];
      data.data.pickListMap.FETCH_CREDIT_DOCUMENT_REF?.map(
  ...
  const customRequest = ({ file, onSuccess }: any) => {
    // Here, you can access the file content and convert it to a byte array
  ...
  const handleSaveDocument = async (e: React.FormEvent) => {
    ...
    if (validator.allValid()) {
      ...
      let data;
      if (props?.docData) {
        data = await modifyCustomerDocumentMutation.mutateAsync(payload);
      } else {
        data = await createCustomerDocumentMutation.mutateAsync(payload);
      }
  ...
  const handleDeleteDocument = async (e: React.FormEvent) => {
    ...
    setModelData({
      ...
      onOk: async () => {
        ...
        setIsLoading(true);
        const data = await removeCustomerDocumentMutation.mutateAsync(payload);
        if (data.status === 200) {
          setIsLoading(false);
          const notificationData: CommonnotificationProps = {
            type: "success",
        ...
        <div className={styles["input-container-split"]}>
          <div>
            <CustomSelector
              label={"Document Type"}
              onChange={(e, option) => {
                handleSelector(e, option, "docTypeDesc", "docTypeId");
              }}
          ...
          >
            <div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <p className={styles["label"]}>Select file to Upload</p>
              ...
              <span className="text-error">
                {validator.message("binaryDocument", byteArray, "required")}
              </span>
            </div>
          </ConfigProvider>
        </div>
