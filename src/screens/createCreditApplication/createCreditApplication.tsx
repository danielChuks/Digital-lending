...
const CreateCreditApplication = () => {
  ...
  const fetchPicklistData = async () => {
    ...
    if (data.status === 200) {
      let pickListMap = [
        ...
        {
          data: data.data.pickListMap.FETCH_COLLATERAL_TYPE_LENDING_REF
        ...
        {
          data: data.data.pickListMap.FETCH_CREDIT_UTILISATION_BY_CREDIT_TYPE
        ...
        {
          data: data.data.pickListMap.FETCH_PRODUCT_BY_CUSTOMER_AND_CREDIT_TYPE
          ,
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if ( CustomerData.customerDraftReadOnlyFlag || currentStep === 1 || currentStep === 2) {
  ...
  const handleCreateCreditApplication = async (e: React.FormEvent) => {
    ...
    if (data.status === 200) {
      ...
      openNotificationWithIcon(notificationData);
      setTimeout(() => { navigate({ pathname: '/dashboard' }) }, 2000);
    }
    else if (data.errorCode === "CI_JWT_001" || data.errorCode === "CI_JWT_002") {
      setIsLoading(false);
      const notificationData: CommonnotificationProps = {
        ...
        <section className={styles["customer-type-content"]}>
          <form onSubmit={handleFormSubmit}>
            ...
            <div className={styles["buton-container"]}>
              ...
              {currentStep === 2  && !CustomerData.customerDraftReadOnlyFlag? (
                <>
