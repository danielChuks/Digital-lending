...
import { ConfigProvider, Divider, Steps, Form, notification } from "antd";
...
const CreateCustomer = () => {
  ...
  const fetchPicklistData = async () => {
    ...
    if (data.status === 200) {
      let pickListMap = [
        { data: data.data.pickListMap.FETCH_TITLES, context: "titleList" },
        {
          data: data.data.pickListMap.FETCH_ADDRESS_TYPE,
          context: "addressTypeList",
        },
        { data: data.data.pickListMap.FETCH_COUNTRY, context: "countryList" },
        {
        ...
        { data: data.data.pickListMap.FETCH_GENDER_REF, context: "genderList" },
        {
        ...
        {
          data: data.data.pickListMap.FETCH_MARKET_CAMPAIGN_REFERENCES,
  ...
  const fetchCustomerTypes = async () => {
    ...
    if (data.status === 200) {
      let pickListMap = [
        {
          data: data.data.pickListMap.FETCH_CUSTOMER_TYPES_BY_CUST_CATOGORY,
        ...
        <section className={styles["customer-type-content"]}>
          <form onSubmit={handleFormSubmit}>
            <>
              ...
              >
                <Steps current={currentStep} items={stepsList} />
              </ConfigProvider>
            </>
