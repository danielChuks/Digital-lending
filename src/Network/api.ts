...
axiosInstance.interceptors.request.use(
  async (request: any) => {
    ...
    request.headers["X-Auth-Signature"] = configData.DBS_WEB_X_AUTH_SIGNATURE;
        ...
        if (refreshTokenResponse.status === 200) {
          ...
          try {
            ...
            if (undefined !== repostResponse) {
              ...
              apiResponse.status = 500;
              apiResponse.errorMessage = "Error in Reposting Response";
            }
            ...
            if (
              ...
              apiResponse.errorCode = repostError.response.data[0].code;
              apiResponse.status = repostError.response.status;
              apiResponse.errorMessage = repostError.response.data[0].message;
            } else {
              apiResponse.status = 500;
...
const refreshToken = async () => {
  ...
  try {
    ...
    if (
      ...
      refreshTokenResponse.errorMessage = refreshError.response.data[0].message;
    } else {
      refreshTokenResponse.status = 500;
      refreshTokenResponse.errorMessage = "Error in Refresh Token Posting";
    }
  }
  return refreshTokenResponse;
};
...
export const fetchConfigInfo = async () => {
  let data;
  try {
    const res = await fetch("/palmsdigitalportal/applicationProperties.json");
    data = await res.json();
  } catch (error) {
